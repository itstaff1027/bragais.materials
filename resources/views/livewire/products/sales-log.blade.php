<div>
    <div class="flex justify-between">
        ...
        {{-- <form class="flex flex-col" wire:submit.prevent='getDataMonthly'>
            <input type=text wire:model='year' placeholder="Order No - 1234" />

            <input type=text wire:model='month' placeholder="Order No - 1234" />
            <button type="submit"> SEARCH </button>
        </form>
        <form class="flex flex-col" wire:submit.prevent='getAllItems'>
            <input type=text wire:model='year' placeholder="Order No - 1234" />
        ...
        <form class="flex flex-col" wire:submit.prevent='getItemsWithCustomer'>
            <input type=text wire:model='year' placeholder="Order No - 1234" />
        ...
        <table class="table-fixed w-full text-center divide-y-2 divide-violet-400 hover:divide-pink-400">
            ...
            <tbody>
                ...
                <tr class="hover:bg-violet-900 hover:text-white odd:bg-gray-200" wire:key:='{{ $order['OrderNo'] }}'>
                    ...
                    <td>{{ $order['Courier'] }}</td>
                    <td>
                        @if ( $order['OrderType'] != 'MTO')
                            @if( $order['Sale'] == 'ONLINE' || $order['Sale'] == 'STORE' )
                            ...
                            @else
                                <form wire:submit.prevent="addToSponsorShip('{{ $order['AgentNo'] }}', '{{ $order['OrderNo'] }}', '{{ $order['FName'] . ' ' . $order['LName'] }}', '{{ $order['OrderList'] }}', '{{ $order['Packaging'] }}', '{{ $order['ClosedSaleDate'] }}', '{{ $order['Remarks'] }}', '{{ $order['DelNotes'] }}', '{{ $order['is_replacement'] }}')"
                                wire:confirm="Are you sure you want to add this to Sponsorship?"
                                >
                                    @csrf
                                    <button class="rounded-lg p-2 bg-cyan-400 hover:bg-cyan-600" type="submit">
                                        SS
                                    </button>
                                    <div wire:loading>
                                        Saving post...
                                ...
                                <form wire:submit.prevent="addToPullOut('{{ $order['AgentNo'] }}', '{{ $order['OrderNo'] }}', '{{ $order['FName'] . ' ' . $order['LName'] }}', '{{ $order['OrderList'] }}', '{{ $order['Packaging'] }}', '{{ $order['ClosedSaleDate'] }}', '{{ $order['Remarks'] }}', '{{ $order['DelNotes'] }}', '{{ $order['is_replacement'] }}')"
                                wire:confirm="Are you sure you want to add this to Pullouts?"
                                >
                                    @csrf
                                    <button class="rounded-lg p-2 bg-sky-400 hover:bg-sky-600" type="submit">
                                        PO
                                    </button>
                                    <div wire:loading>
                                        Saving post...
                            ...
                            @endif
                        @else
                            <form wire:submit.prevent="addToMTO(
                                '{{ $order['AgentNo'] }}', 
                                '{{ $order['OrderNo'] }}', 
                                '{{ $order['FName'] . ' ' . $order['LName'] }}', 
                                '{{ $order['OrderList'] }}', 
                                '{{ $order['Packaging'] }}', 
                                '{{ $order['ClosedSaleDate'] }}', 
                                '{{ $order['Remarks'] }}', 
                                '{{ $order['DelNotes'] }}', 
                                '{{ $order['is_replacement'] }}'
                            )"
                            wire:confirm="Are you sure you want to add this to MTO?"
                            >
                                @csrf
                                <button class="rounded-lg p-2 bg-orange-500 hover:bg-orange-600" type="submit">
                                    MTO
                                </button>
                                <div wire:loading>
                                    Saving post...
